...
def sunsetrise(address, datum=None, zone=None, kind="sunrise"):
    ...
    dates = [datetime(datum.year, month=3, day=20), datetime(datum.year, month=6, day=21),
             datetime(datum.year, month=10, day=22),
             datetime(datum.year, month=12, day=21)]
    map(lambda d: abs(d - datum), dates)
    eqtime = 229.18 * (
            0.000075 + 0.001868 * cos(fracy) - 0.032077 * sin(fracy) - 0.014615 * cos(2 * fracy) - 0.040849 * sin(
        2 * fracy))
    date2lengt = {datetime(datum.year, month=3, day=20): 0, datetime(datum.year, month=6, day=21): pi / 2,
...
def astronomy(datum=None, save=False):
    ...
    if apod["media_type"] == "image" and save:
        if "hdurl" in apod.keys():
            title = apod["title"].replace(" ", "_").replace(":", "_") + "_" + datum.strftime("%d-%m-%Y") + ".jpg"
            if type(save) == str:
                image_dir = save
            else:
                image_dir = "apod"
